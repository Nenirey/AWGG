...
copy awgg.wxs           %BUILD_PACK_DIR%\
copy ..\..\src\awgg.ico %BUILD_PACK_DIR%\

pushd %BUILD_PACK_DIR%

move awgg "AWGG"
heat dir "AWGG" -ag -cg HeatGroup -dr %PF% -var var.SourcePath -o include.wxs
candle -arch %CPU_TARGET% -dProductVersion=%AWGG_VER% -dSourcePath="AWGG" -dProgramFiles=%PF% awgg.wxs include.wxs
light -ext WixUIExtension -cultures:en-us include.wixobj awgg.wixobj -o %PACKAGE%.msi
